...
export default async function RWASetupPage({ params }: Props) {
  ...
  if (!res.ok) {
    ...
    return (
      <div className="min-h-screen bg-oc-inset flex items-center justify-center px-4">
        <div className="text-center space-y-4">
          <div className="w-16 h-16 rounded-full bg-red-500/10 flex items-center justify-center mx-auto">
            <span className="text-red-400 text-2xl">✕</span>
          </div>
          <h1 className="text-xl font-display font-bold text-oc-fg tracking-[-0.01em]">Link Error</h1>
    ...
    <div className="min-h-screen bg-oc-inset py-12 px-4">
      <div className="max-w-2xl mx-auto space-y-8">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-display font-bold tracking-[-0.02em] text-oc-fg-soft">Set up your gym</h1>
          <p className="text-sm text-oc-fg-dim">
            Welcome, {lead.contactName}! Complete the form below to set up your CultSport gym at{" "}
            <strong className="text-oc-fg-soft">{lead.societyName}</strong>.
          </p>
        </div>
        <RWASetupShell
          leadId={lead.id}
