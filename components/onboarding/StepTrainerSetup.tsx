...
export function StepTrainerSetup({ data, onChange, onNext, onBack, availableTrainers }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-base font-semibold text-oc-fg mb-1">Map Trainers</h2>
      ...
      <div className="space-y-2">
        {availableTrainers.length === 0 ? (
          <div className="rounded-xl border border-dashed border-oc-border p-8 text-center">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  >
                    {isSelected && <Check className="h-3 w-3 text-oc-base" strokeWidth={2.5} />}
                  </div>
                  <span className="text-sm font-medium text-oc-fg">{trainer.name}</span>
                </div>
      ...
      {selected.length > 0 && (
        <p className="text-xs text-oc-fg-muted">
