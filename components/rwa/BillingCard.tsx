    ...
    <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 overflow-hidden">
      ...
      <div className="flex items-start gap-3 px-5 py-4 border-b border-emerald-500/10">
        ...
        <div>
          ...
          <p className="text-xs text-oc-fg-dim mt-0.5">{centerName}</p>
          {quote.acceptedAt && (
            <p className="flex items-center gap-1 text-[11px] text-oc-fg-dim mt-0.5">
      ...
      <div className="px-5 py-3 border-b border-emerald-500/10">
        <p className="text-[10px] font-semibold text-oc-fg-dim uppercase tracking-wider mb-2">
      ...
      <div className="px-5 py-4 space-y-2">
        {isTotal && quote.totalAmount ? (
          <div className="flex justify-between items-baseline">
            <span className="text-sm text-oc-fg-muted">Total Agreed Amount</span>
          ...
          <>
            {totalOneTime > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-oc-fg-muted">One-time Setup</span>
                <span className="font-bold text-oc-fg">{formatPaise(totalOneTime)}</span>
              </div>
            )}
            {totalMonthly > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-oc-fg-muted">Monthly</span>
                <span className="font-bold text-oc-fg">{formatPaise(totalMonthly)}<span className="text-oc-fg-dim text-xs font-normal">/mo</span></span>
              </div>
            )}
          </>
        )}
        ...
        {quote.notes && (
          <p className="text-[11px] text-oc-fg-dim pt-1 border-t border-emerald-500/10 italic">
